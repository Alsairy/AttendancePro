...
BACKUP_DIR="/var/backups/attendancepro"
...
echo "Starting AttendancePro backup at $(date)"
...
if [ -d "/var/log/attendancepro" ]; then
    tar -czf "$BACKUP_DIR/logs_$DATE.tar.gz" /var/log/attendancepro/
fi
...
if [ -d "/var/lib/attendancepro/uploads" ]; then
    echo "Backing up file uploads..."
    tar -czf "$BACKUP_DIR/uploads_$DATE.tar.gz" /var/lib/attendancepro/uploads/
fi
...
if [ -n "$AWS_S3_BUCKET" ]; then
    ...
    aws s3 sync "$BACKUP_DIR" "s3://$AWS_S3_BUCKET/attendancepro-backups/" --exclude "*" --include "*_$DATE.*"
fi

if [ -n "$AZURE_STORAGE_ACCOUNT" ]; then
    echo "Uploading backups to Azure Storage..."
    az storage blob upload-batch --destination attendancepro-backups --source "$BACKUP_DIR" --pattern "*_$DATE.*"
fi
